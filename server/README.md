...
| `STAMP_PROMPTS_DIR` | No (Phase A only) | `/etc/stamp/reviewers` | Phase A bundled-prompts fallback path. **IGNORED when `STAMP_PROMPTS_REPO_URL` is set** — Phase B mode reads from the cache root above. See the Phase A "Reviewer prompts" section above for the production-safety warning. |
