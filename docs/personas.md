...
> Your prompt context names the base and head SHAs of the diff. Before reviewing, run `git log --format='%B' BASE..HEAD` via `Bash` (substituting those two SHAs) to read the branch's commit messages — `git diff` output does **not** include them. If the messages or the diff mention a ticket ID matching `[A-Z]+-[0-9]+`, use the `linear` MCP to fetch that ticket and evaluate whether the diff satisfies its acceptance criteria. If the ticket is missing or the scope doesn't match, flag it. (Bound the lookup: one fetch per ticket, no chasing sub-tasks — the reviewer runs under a turn cap.)

Security tradeoffs worth naming: granting tools expands what a malicious reviewer prompt can do. See DESIGN.md's security model for the full discussion — the short version is `Read`/`Grep`/`Glob` are sandboxed to `repoRoot` by an explicit allowlist enforced in the SDK's `hooks.PreToolUse` callback (paths that resolve outside the repo are denied, and a denylist of reviewer-internal targets — all of `.git/` (incl. `.git/config` credentials and the attestation DB) and `.stamp/trusted-keys/` — is denied even inside the repo, for Read/Grep/Glob alike), `WebFetch` is host-allowlisted by the same hook against `allowed_hosts` from `.stamp/config.yml` (with optional `path_prefix:` to additionally pin the URL path on a per-host basis) and can otherwise exfiltrate diff contents if the prompt is hostile, and MCP servers run as subprocesses with whatever permissions their binaries ask for. Treat `tools:` and `mcp_servers:` additions in `.stamp/config.yml` as security-sensitive diffs; calibrate your security reviewer's prompt to flag them explicitly.
