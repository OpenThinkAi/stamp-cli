...
describe("verdict-cache invalidation on prune (AGT-697 / issue #58)", () => {
  ...
  // AGT-1137: provenance joined the key too. These tests are about the
  // watermark, so one backend is held constant across seed and lookup.
  ...
  it("findCachedVerdict skips rows at/before the watermark, keeps newer rows", () => {
    ...
    try {
      ...
      assert.equal(
        findCachedVerdict(
          db, REVIEWER, DIFF_HASH, PROMPT_HASH, TREE_SHA, headOf("h-old"), BRANCH,
          PROVENANCE,
        )?.verdict,
        "changes_requested",
      );
      ...
      assert.equal(
        findCachedVerdict(
          db, REVIEWER, DIFF_HASH, PROMPT_HASH, TREE_SHA, headOf("h-fresh"), BRANCH,
          PROVENANCE,
        )?.verdict,
        "approved",
  ...
  it("runPrune bumps the watermark so a within-cutoff stale row no longer cache-hits (the #58 repro)", () => {
    ...
    try {
      assert.equal(
        findCachedVerdict(
          db, REVIEWER, DIFF_HASH, PROMPT_HASH, TREE_SHA, headOf("h-recent"), BRANCH,
          PROVENANCE,
        )?.verdict,
        "changes_requested",
      );
