...
Read by the server-side post-receive hook after a push is accepted. For each configured branch, the hook pushes to GitHub through one of three transports, selected in this order:

1. **Per-repo SSH deploy key** (preferred, current default). The hook looks for `/srv/git/.ssh-client-keys/<owner>_<repo>_ed25519`, generated on demand by the server-side `stamp-server-pubkey <owner>/<repo>` wrapper (via a sudo-elevated `stamp-ensure-repo-key` helper that refuses to overwrite existing files). When present, the push goes over SSH with `GIT_SSH_COMMAND="ssh -F /dev/null -i <key> -o IdentitiesOnly=yes ..."` so the static client config's IdentityFile is bypassed and only the per-repo key is offered to github.com. The Ruleset's `DeployKey` bypass-actor whitelists exactly this key — see `docs/github-ruleset-setup.md` for the operator-facing setup.
2. **Legacy shared SSH deploy key** at `/srv/git/.ssh-client-keys/github_ed25519`. Pre-per-repo-keys design. Falls through when no per-repo key file exists. Retained as back-compat so repos provisioned before the per-repo model survive a server upgrade.
3. **HTTPS with `GITHUB_BOT_TOKEN`** (original transport). When neither SSH key is installed, the hook pushes to `https://github.com/<repo>.git` with `http.extraHeader: Authorization: Basic base64(x-access-token:<PAT>)` supplied via `GIT_CONFIG_*` env vars (not the URL — that form would leak the token to `/proc/<pid>/cmdline`). `GITHUB_BOT_TOKEN` comes from `/etc/stamp/env` on the server (populated by the entrypoint from a Railway env var; sshd strips custom env vars from sessions, so a file is the reliable transport).

Why three transports rather than just SSH: deploy-key creation can be disabled at the org-policy level on some locked-down work organizations, and the HTTPS+PAT path is the recovery option there. The token is still required for the post-push `stamp/verified` status REST calls regardless of which push transport was selected.

The same logic applies to tag pushes whose ref name matches one of the `tags:` patterns (added in 0.7.8 — repos that publish on tag push, like npm/Cargo/PyPI release workflows, need this so the action fires). Mirror failures log to stderr but don't block the stamped push.

**Per-repo deploy-key trust property.** GitHub's Ruleset `DeployKey` bypass actor is **repo-scoped, not key-id-scoped**: any deploy key registered on the mirror repo bypasses the ruleset, regardless of which specific id the bypass actor names. (At the API level, you POST `actor_id: <id>` but it round-trips back as `actor_id: null`.) The defense is operational: only the canonical `stamp-mirror` deploy key should ever exist on a mirror repo. The `stamp provision` / `--migrate-bypass` flows uphold this invariant by deleting any prior `stamp-mirror` entry whose body doesn't match the server's current per-repo pubkey before re-registering — but a CI deploy key added later for an unrelated purpose would silently become a bypass actor too.

**Container-restart caveat.** Per-repo deploy keys are generated lazily on first request and persist on the server's `/srv/git` volume. Hosting platforms that restart the container without strict volume preservation may regenerate the per-repo files on next request, invalidating every `stamp-mirror` key registered on a GitHub mirror. The legacy shared key persists across restarts because the entrypoint re-creates it if missing; per-repo files have no equivalent boot-time regeneration. The operator recovery is `stamp provision --migrate-bypass`, which detects the body mismatch and re-registers automatically. A future change may add a `.stamp/mirror.yml`-scan in the entrypoint to lazy-regenerate per-repo keys at boot — until then, document the recovery flow as part of post-redeploy operator routine.

**Key-file ownership posture.** Per-repo deploy-key files (and the legacy shared key) are owned `git:git` mode `0600` — the standard SSH posture, owner-only readable, accepted by OpenSSH 9.x's client-side strict-perms check. The entrypoint script enforces this on every boot via a scan over `/srv/git/.ssh-client-keys/*_ed25519`, fixing drift from earlier helper versions or backups that didn't preserve mode. An earlier design used `root:git 0640` (root-owned, group-readable) to prevent a git-shell escape from substituting a key whose public half the attacker holds, but OpenSSH 9.x's strict-perms check rejects any private key with group bits set, breaking the SSH transport. The migration to `git:git 0600` trades that local substitution defense for SSH-client compatibility; the attacker still needs GitHub repo-admin auth to register a different key as the bypass actor on the mirror, so the practical exploit chain remains multi-step rather than single-step.
