 ...
 * (AGT-337) and assemble the verified subset for the v4 envelope.
 *
 * Inputs are exactly the fields needed to reconstruct the signing
 * target — the same canonical bytes `verifyV4TrustAnchorSignatures` in
 * `src/hooks/pre-receive.ts` will check against on push. By verifying
 * here we both (a) reject stale notes whose signatures don't match the
 * fresh payload, and (b) refuse to land a merge that the server would
 * reject, surfacing the failure on the operator's machine with a clean
 * recovery path.
 *
 * Path-rule discovery: we read path_rules at base_sha (matching the
 * verifier's snapshot semantics, settled decision #5). If no rule
 * matches the diff, we return [] and short-circuit — the diff isn't
 * gated by `path_rules`, the verifier won't require sigs, and a stale
 * note from a previous iteration doesn't get folded in.
 */
function collectTrustAnchorSignatures(input: {
  repoRoot: string;
  baseSha: string;
  headSha: string;
  targetBranch: string;
  ...
  if (!input.manifest) {
    throw new Error(
      `cannot fold trust-anchor signatures: manifest at base ${input.baseSha.slice(0, 8)} failed to parse.`,
    );
  }
  for (const sig of note.signatures) {
    ...
    if (!pem) {
      failures.push(
        `no .pub file at base for ${sig.signer_key_id}`,
      );
      continue;
    }
    let ok = false;
  ...
  // applied here so a too-low count surfaces at merge time with a
  // clean operator message rather than via a pre-receive rejection
  // after the merge commit already exists.
    ...
    if (qualifying < rule.minimum_signatures) {
      ...
      throw new Error(
        `v4 path_rules["${rule.pattern}"] requires ${rule.minimum_signatures} admin signature(s) with capability '${rule.require_capability}', ` +
          ...
          `Collect more with \`stamp admin sign --pending ${input.headSha.slice(0, 12)}\` (and have admins re-sign if previously stale), then re-run \`stamp merge\`.`,
      );
    }
  }
 ...
 *  in-file rather than importing the verifier's private helper —
 *  smaller surface area than restructuring the hook for export. */
